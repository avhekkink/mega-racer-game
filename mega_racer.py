...
class RenderingSystem:
    ...
    vec3 computeShading(vec3 materialColour, vec3 viewSpacePosition, vec3 viewSpaceNormal, vec3 viewSpaceLightPos, vec3 lightColour)
    {
        ...
        // Using lab 4 as a template, I first compute the normalized direction towards the light.
        vec3 viewSpaceDirToLight = normalize( viewSpaceLightPos - viewSpacePosition);
        // Next, I normalize the surface normal, which is stored in viewSpaceNormal
        vec3 viewSpaceNormalizedNormal = normalize(viewSpaceNormal);
        // Then, I compute the incoming light intensity using that it is proportional to the cosine of the angle between the surface normal and direction to the light
        // I utilise the dot product to achieve this, and clamp the result at zero to prevent weird looking artefacts
        float incomingIntensity = max(0.0, dot(viewSpaceNormalizedNormal, viewSpaceDirToLight));

        // I set the incoming light as the light colour multiplied by its intensity
        vec3 incomingLight = incomingIntensity * lightColour;

        // To prevent things being pitch black when not facing the light, I add a constant ambient lighting term
        vec3 totalIncomingLight = incomingLight + globalAmbientLight;

        // Multiply our material colour by this lambertian lighting effect, and output the result
        vec3 outputColour = materialColour * totalIncomingLight;
        return outputColour;
    }
    """
    objModelShader = None
...
def update(dt, keyStateMap, mouseDelta):
    ...
    # Camera position is set to the racer.position with an offset in the negative 'heading' direction ( so that the camera always follows behind it) and in the positive z-direction (up)
    # CamOffsetBehind stores the x,y,z component needed to offset the camera directly behind the racer by normalising the heading vector and multiplying by g_followCamOffset to get the desired magnitude
    CamOffsetBehind = g_followCamOffset * normalize(g_racer.heading)
    CamOffsetAbove = g_followCamOffset
    g_viewPosition = g_racer.position - CamOffsetBehind + [0, 0, CamOffsetAbove]
    # Camera target is set to the racer.position with an offset in the positive z-direction (up)
    g_viewTarget = g_racer.position + [0, 0, g_followCamLookOffset]
