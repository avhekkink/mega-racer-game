...
class Terrain:
    ...
    def render(self, view, renderingSystem):
        ...
        # Enable access from the shader to the texture by setting the sampler uniforms to the same units as the texture
        # Using the code skeleton from lab 5
        # Then, bind our loaded grass texture to the TU_Grass texture unit
        # This binds the grass texture to the '0th' texture unit (until something else is bound there)
        lu.bindTexture(self.TU_Grass, self.grassTexId)
        lu.setUniform(self.shader, "grassTexId", self.TU_Grass)
...
"""
        ...
        fragmentShader = """
            ...
            uniform sampler2D GrassTexId;

            out vec4 fragmentColor;

            void main() 
            {
                // Note: Old material colour is no longer needed, hence commented out
                ...
                // We are using world space coordinates as texture coordinates, and so, to scale them we simply need to multiply the world space coords by the terrainTextureXyScale
                // BUT, the texture is 2D so we only need the xy coordinates of the world space, we store these in textureCoord
                // Then, use texture() to retrieve the texels from GrassTexId and apply them to the scaled textureCoord
                // And finally convert this to a 3D vector using .xyz so it can be stored in vec3 materialColour and used in vec3 reflectedLight
                vec2 textureCoord = v2f_worldSpacePosition.xy;
                vec3 grassColour = texture(GrassTexId, textureCoord * terrainTextureXyScale).xyz;
                // Set the new material colour as the grass colour
...
"""
        ...
        # ObjModel.loadTexture loads our texture and creates an OpenGL texture
        basePath = ""
        self.grassTexId = ObjModel.loadTexture("data/grass2.png", basePath, True)
